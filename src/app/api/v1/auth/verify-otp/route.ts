...
async function verifyOtp(request: NextRequest) {
  try {
    ...
    const { phone, otp, name } = verifyOtpSchema.parse(body)
    ...
    if (!user) {
      ...
      user = await db.user.create({
        data: {
          ...
          paymentActive: false,
          isFrozen: false,
          referralCode: `H2E${Math.random().toString(36).substring(2, 8).toUpperCase()}`
        }
      })
    ...
    await db.activityLog.create({
      data: {
        userId: user.id,
        action: 'LOGIN',
        details: `OTP verification successful${isNewUser ? ' (new user)' : ''}`
      }
    })
