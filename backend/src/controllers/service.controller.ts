...
/**
 * PUBLIC LIST (Discovery Engine)
 * Filters: Zone, Category, SubCategory, Availability, Rating
 */
export const listServices = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { zoneId, templateId, page = 1, limit = 20 } = (req as any).query;
    const offset = (Number(page) - 1) * Number(limit);
    
    // Strict Filtering: Active, Approved, LIVE Provider, Active Template
    let query = `
       SELECT s.id, s.price, 
    ...
    // Default Ranking: Rating DESC, Price ASC
    ...
    // Remove full_count from individual rows
...
/**
 * CREATE SERVICE (Strict Catalog Mode)
 */
export const createService = async (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (!template_id || !zone_id || !price) {
        return (res as any).status(400).json({ message: "Missing required fields (Template, Zone, Price)" });
    }

    await client.query("BEGIN");
    
    // 1. Validate Provider
    const providerCheck = await client.query(`SELECT verification_status FROM users WHERE id=$1`, [providerId]);
    if (providerCheck.rowCount === 0) throw new Error("Provider not found");
    // We allow service creation before full approval (part of onboarding), but it stays inactive

    // 2. Validate Template & Price Guardrails
    const tmplRes = await client.query(`SELECT * FROM service_templates WHERE id=$1`, [template_id]);
    if (tmplRes.rowCount === 0) return (res as any).status(404).json({ message: "Service Template not found" });
    const tmpl = tmplRes.rows[0];

    if (tmpl.min_price && price < tmpl.min_price) {
        return (res as any).status(400).json({ message: `Price too low. Minimum allowed: ${tmpl.min_price}` });
    }
    if (tmpl.max_price && price > tmpl.max_price) {
        return (res as any).status(400).json({ message: `Price too high. Maximum allowed: ${tmpl.max_price}` });
    }
...
export const updateService = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        // Should optionally re-validate price against template limits if updated
        await db.query(
...
export const approveService = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        await db.query(`UPDATE services SET is_approved=true, is_active=true WHERE id=$1`, [(req as any).params.id]);
...
export const rejectService = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        await db.query(`UPDATE services SET is_approved=false, is_active=false, reject_reason=$1 WHERE id=$2`, [(req as any).body.reason, (req as any).params.id]);
