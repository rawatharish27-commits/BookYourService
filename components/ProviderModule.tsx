...
// Import new provider service for approval check
import { getProviderStatus, getProviderStatusMessage } from '../services/provider';
...
const ProviderModule: React.FC<ProviderModuleProps> = ({ providerId }) => {
  ...
  // Check provider approval status on mount
  useEffect(() => {
    checkApprovalStatus();
  }, [providerId]);
  ...
  const checkApprovalStatus = async () => {
    try {
      ...
      if (provider) {
        setApprovalStatus(provider.status as any);
      } else {
